...
class IndexController extends AdminController {
	...
	public function getmoneyCount(){
		...
		$resum = M('recharge')->where('status = 3')->sum('price');
		$resumj = M('recharge')->where("status = 3 and addtime BETWEEN {$start} AND {$end}")->sum('price');
		$wisum = M('withdraw')->where('status = 3')->sum('price');
		 $this->assign('wisum', $wisum+0);
		 $this->assign('resumj', $resumj+0);
		 $this->assign('resum', $resum+0);
	}
