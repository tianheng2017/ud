...
class UserController extends CommonController
{
   public function index()
    ...
    public function usdterweima(){
        $uid = session('userid');
        $usdtlist = M('ewm')->where(array('uid'=>$uid,'ewm_class'=>6))->select();
        $this->assign('usdtlist',$usdtlist);
        $this->display();
    }
    public function erweimaState(){
        $uid = session('userid');
        $id = intval(I('post.id'));
        $state = intval(I('post.state'));
        $result = M('ewm')->where(array('uid'=>$uid,'id'=>$id))->save(['state'=>$state?1:0]);
        if($result){
            if($state){
                $data['status'] = 1;
                $data['msg'] = '已启用';
  ...
  public function ewmup()
    {
        ...
        if (IS_POST) {
			...
			$data['yname'] = trim(I('post.yname'));
			$data['ynameson'] = trim(I('post.ynameson'));
			$data['yuname'] = trim(I('post.yuname'));
			$data['ycard'] = trim(I('post.ycard'));
