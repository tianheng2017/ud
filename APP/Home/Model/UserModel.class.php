...
class UserModel extends ModelModel
{
	...
	protected $_validate = array(
		...
		//asam
		//array('mobile', '/^1[3456798]\d{9}$/', '手机号码格式不正确(37)', self::MUST_VALIDATE, 'regex', self::MODEL_INSERT),
	...
	public function login($account, $password, $map = null)
	{
		...
		if (!$user_info) {
            ...
            $this->error = '您的账号已锁定，请联系管理员';
            return false;
        } elseif ($user_info['real'] == 0) {
            $this->error = '账号正在审核中，请耐心等待';
            return false;
        } elseif ($user_info['real'] == -1) {
            $this->error = '未通过实名制审核';
            return false;
		} else {
